...
- (void) updateDatabase {
    ...
    switch (dbVersion) {
        ...
        case YYIM_DB_VERSION_16:
            [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
                [db executeUpdate:YYIM_CHATGROUP_ADD_CREATOR];
                [db executeUpdate:YYIM_CHATGROUP_ADD_CONTENT];
                [db executeUpdate:YYIM_CHATGROUP_ADD_ANNOUN_TS];
                [db executeUpdate:YYIM_DBINFO_UPDATE, [NSNumber numberWithInteger:YYIM_DB_VERSION_17]];
            }];
    }
}
...
- (void)batchUpdateRoster:(NSArray *)rosterArray {
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        @try {
            while ([rs next]) {
                [rosterIdArray addObject:[YYIMStringUtility notNilString:[rs stringForColumn:@"roster_id"]]];
            }
        }
        @finally {
            [rs close];
...
- (void)insertRosterTags:(NSArray *)tagArray rosterId:(NSString *)rosterId {
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        @try {
            while ([rs next]) {
                [oldTagArray addObject:[YYIMStringUtility notNilString:[rs stringForColumn:@"tag"]]];
            }
        }
        @finally {
            [rs close];
...
- (NSArray *)getRosterTagsWithRosterId:(NSString *)rosterId db:(YYFMDatabase *)db{
    ...
    @try {
        while ([rs next]) {
            [array addObject:[YYIMStringUtility notNilString:[rs stringForColumn:@"tag"]]];
        }
    }
    @finally {
        [rs close];
...
- (NSDictionary *)getAtCountDic:(YYFMDatabase *)db {
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    NSString *sql = @"select count(pkid) at_count,room_id from yyim_message where self_id in (?,?) and direction=? and status=? and isat=1 group by room_id";
    NSMutableArray *argsArray = [NSMutableArray array];
    [argsArray addObject:[[YYIMConfig sharedInstance] getFullUserAnonymousSpecialy]];
    NSString *allUserStr = [NSString stringWithFormat:@"%@.%@.%@",[YYIMStringUtility getAllUserId],[[YYIMConfig sharedInstance] getAppKey], [[YYIMConfig sharedInstance] getEtpKey]];
    [argsArray addObject:allUserStr];
    [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_DIRECTION_RECEIVE]];
    [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_STATE_NEW]];
...
- (void)updateMessageDeliveredWithPacketVersion:(NSInteger)packetVersion {
    if (packetVersion <= 0) {
        return;
    }
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        NSMutableArray *argsArray = [NSMutableArray array];
        [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_STATE_DELIVERED]];
        [argsArray addObject:[NSNumber numberWithInteger:packetVersion]];
        [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_STATE_DELIVERED]];
        [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_STATE_SENT_OR_READ]];
        [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_DIRECTION_SEND]];
        [argsArray addObject:[[YYIMConfig sharedInstance] getFullUserAnonymousSpecialy]];
        NSString *sql = @"update yyim_message set status=? where version<? and status<? and status>=? and direction=? and self_id=?";
        [db executeUpdate:sql withArgumentsInArray:argsArray];
    }];
}
...
- (YYMessage *)revokeMessageWithPid:(NSString *)pid {
    __block BOOL isExtend = [[YYIMConfig sharedInstance] isNotRevokeExtend];
    [[self getDBQueue] inDatabase:^(YYFMDatabase *db) {
        NSMutableArray *argsArray = [NSMutableArray array];
        [argsArray addObject:[NSNumber numberWithInteger:YM_MESSAGE_STATE_SENT_OR_READ]];
        ...
        NSString *sql = @"";
        if (isExtend) {
            sql = @"UPDATE yyim_message SET status=?,key_info=?,type=?,res_local=?,res_thumb_local=?,res_original_local=? WHERE self_id=? and packet_id=?";
        } else {
            sql = @"UPDATE yyim_message SET status=?, message=?,key_info=?,type=?,res_local=?,res_thumb_local=?,res_original_local=? WHERE self_id=? and packet_id=?";
...
- (void)batchUpdateChatGroup:(NSArray *)chatGroupArray {
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        @try {
            while ([rs next]) {
                [groupIdArray addObject:[YYIMStringUtility notNilString:[rs stringForColumn:@"chatgroup_id"]]];
            }
        }
        @finally {
            [rs close];
...
- (void)updateChatGroup:(YYChatGroup *)group {
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        if (existGroup) {
            ...
            NSArray *argsArray = [self chatGroupUpdateArgsArray:group];
            [db executeUpdate:YYIM_CHATGROUP_UPDATE_NEW withArgumentsInArray:argsArray];
            [self updateChatGroupTags:db group:group];
        } else {
            NSArray *argsArray = [self chatGroupInsertArgsArray:group];
            [db executeUpdate:YYIM_CHATGROUP_INSERT_NEW withArgumentsInArray:argsArray];
            [self updateChatGroupTags:db group:group];
        }
    }];
}
...
- (void)batchUpdatePubAccount:(NSArray *)accountArray {
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        @try {
            while ([rs next]) {
                [accountIdArray addObject:[YYIMStringUtility notNilString:[rs stringForColumn:@"account_id"]]];
            }
        }
        @finally {
            [rs close];
...
- (void)fillChatGroup:(YYChatGroup *)group withResultSet:(YYFMResultSet *)rs {
    ...
    [group setAnnouncementCreator:[rs stringForColumn:@"chat_announ_creator"]?:@""];
    [group setAnnouncementContent:[rs stringForColumn:@"hat_announ_content"]?:@""];
    [group setAnnouncementTs:[rs longLongIntForColumn:@"hat_announ_ts"]];
}
...
- (NSArray *)chatGroupInsertArgsArray:(YYChatGroup *)group {
    ...
    [argsArray addObject:[YYIMStringUtility notNilString:[group announcementCreator]]];
    [argsArray addObject:[YYIMStringUtility notNilString:[group announcementContent]]];
    [argsArray addObject:[NSNumber numberWithLongLong:[group announcementTs]]];
