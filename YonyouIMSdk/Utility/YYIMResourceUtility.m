...
+ (NSString *)resourceAttachRelaPathWithId:(NSString *)attachId ext:(NSString *)ext {
    ...
    // 文件路径
    NSString *fileName = [attachId stringByAppendingPathExtension:ext];
...
+ (NSString *)getAttachKey:(NSString *)attachId imageType:(YYIMImageType)imageType {
    switch (imageType) {
        case kYYIMImageTypeOriginal:
            return [NSString stringWithFormat:@"%@_%@", attachId, @"original"];
        case kYYIMImageTypeThumb:
            return [NSString stringWithFormat:@"%@_%@", attachId, @"thumb"];
        default:
            return attachId;
    }
}
...
+ (NSString *)getThumbAttachKey:(NSString *)attachId {
    return [NSString stringWithFormat:@"%@_thumb", attachId];
}
