...
@property (strong, nonatomic) YYUserProfileSetting *userProfileSetting;
/**
 *  当前正在创建群组的视图
 */
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (!(tableView == self.searchDisplayController.searchResultsTableView)) {
        ...
        if ([[YYIMTargetManager sharedManager] allowWallspace] && [chatType isEqualToString:YM_MESSAGE_TYPE_PUBACCOUNT] && [chatId isEqualToString:@"wallspace"]) {
            WallSpaceViewController *wallSpaceViewController = [[WallSpaceViewController alloc] initWithNibName:nil bundle:nil];
            wallSpaceViewController.page = @"notices";
            UINavigationController *navController = [[UINavigationController alloc] initWithNavigationBarClass:[WallSpaceNavigationBar class] toolbarClass:nil];
            [navController addChildViewController:wallSpaceViewController];
            [YYIMUtility genThemeNavController:navController];
            [self presentViewController:navController animated:YES completion:nil];
            [[YYIMChat sharedInstance].chatManager updateMessageReadedWithId:chatId];
        } else if ([chatId isEqualToString:YM_NETCONFERENCE_PUBACCOUNT] && [chatType isEqualToString:YM_MESSAGE_TYPE_PUBACCOUNT]) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (self.userProfileSetting.silenceSwitch) {
        return 40;
    }
    return 0;
}
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.recentTableView.frame), 40)];
    
    if (self.userProfileSetting.silenceSwitch) {
        view.backgroundColor = [UIColor groupTableViewBackgroundColor];
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 0, 23, 21)];
        imageView.image = [UIImage imageNamed:@"yyim_silence_small"];
        [view addSubview:imageView];
        CGPoint point = imageView.center;
        imageView.center = CGPointMake(point.x, CGRectGetHeight(view.frame) / 2);
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(60, 0, CGRectGetWidth(view.frame) - 60, CGRectGetHeight(view.frame))];
...
- (NSArray *)menuDataDicArray {
    if (!self.menuArray) {
        ...
        if (self.userProfileSetting.silenceSwitch) {
            [array addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"yyim_silence_small", @"icon", @"退出沉浸", @"name", nil]];
        } else {
            [array addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"yyim_silence_small", @"icon", @"沉浸模式", @"name", nil]];
        }
...
- (void)didSelectMenuAtIndex:(NSUInteger)index {
    switch (index) {
        ...
        case 7:{
            [[YYIMChat sharedInstance].chatManager updateSilenceMode:!self.userProfileSetting.silenceSwitch complete:nil];
        }
        default:
            break;
    }
...
- (void)initData:(NSArray *)messages {
    self.userProfileSetting = [[YYIMChat sharedInstance].chatManager getUserProfileSetting];
    [self menuDataDicArray];
