...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    switch (section) {
        case 0:
            return 1;
        case 1:
            if ([[YYIMTargetManager sharedManager] allowWallspace] && [[YYIMTargetManager sharedManager] allowCloudPan]) {
                return 4;
            } else if ([[YYIMTargetManager sharedManager] allowWallspace] || [[YYIMTargetManager sharedManager] allowCloudPan]) {
                return 3;
            } else {
                return 2;
            }
    ...
    switch (indexPath.section) {
        ...
        case 1:
            ...
            switch (indexPath.row) {
                case 0:
                    if ([[YYIMTargetManager sharedManager] allowCloudPan]) {
                        [cell setHeadIcon:@"icon_cloud"];
                        [cell setName:@"云盘"];
                    } else if ([[YYIMTargetManager sharedManager] allowWallspace]) {
                        [cell setHeadIcon:@"icon_wallspace"];
                        [cell setName:@"我的圈子"];
                    } else {
                    ...
                    break;
                case 1:
                    if ([[YYIMTargetManager sharedManager] allowCloudPan] && [[YYIMTargetManager sharedManager] allowWallspace]) {
                        [cell setHeadIcon:@"icon_wallspace"];
                        [cell setName:@"我的圈子"];
                    } else if ([[YYIMTargetManager sharedManager] allowCloudPan] || [[YYIMTargetManager sharedManager] allowWallspace]) {
                        [cell setHeadIcon:@"icon_mynetmeeting"];
                        [cell setName:@"我的会议"];
                    } else {
...
- (void)openCloudPan {
    PanTabBarController *panTabViewController = [[PanTabBarController alloc] initWithNibName:nil bundle:nil];
    UINavigationController *panNavController = [YYIMUtility themeNavController:panTabViewController];
    [self presentViewController:panNavController animated:YES completion:nil];
}
