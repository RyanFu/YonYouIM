 ...
 *
 *  @param accountId 公共号id
 *  @param msgModel  是否允许客户端发消息 0不允许 1允许
 *  @return 公共号发送消息接口
 */
 ...
 *
 *  @param groupId 群组id
 *  @param packetId  统计的消息id
 *  @return @some 拉取已读的人的Id，@的成员接口
 */
...
/**
 ...
 @return 是否不撤销extend字段
 */
- (BOOL)isNotRevokeExtend;
/**
 设置是否不撤销extend字段

 @param revokeExtend  是否不撤销extend字段
 */
...
/**
 获取typeId honorType传递需要contentType类型为2(文本类型),否则不传递

 @param contentType 基本类型（文本text、语音audio、图片imgae、视频video、文件file、位置location）荣耀等类型（荣耀uculture_honor、拍砖uculture_unhonor、猜一下uculture_havefun、投一下uculture_vote、学一下uculture_learn、顺一下uculture_carshare、发起跳转uculture_challenge、会议uculture_meeting）
 @return 返回typeId
 */
- (NSString *)getTypeIdWithContentType:(NSInteger)contentType;
/**
 设置typeId honorKey传递需要contentKey类型为2(文本类型),否则不传递

 @param value value
 @param contentKey 基本类型（文本text、语音audio、图片imgae、视频video、文件file、位置location）荣耀等类型（荣耀uculture_honor、拍砖uculture_unhonor、猜一下uculture_havefun、投一下uculture_vote、学一下uculture_learn、顺一下uculture_carshare、发起跳转uculture_challenge、会议uculture_meeting）
 */
- (void)setTypeIdWithValue:(NSString *)value contentKey:(NSInteger)contentKey;
@end
