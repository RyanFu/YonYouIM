...
@property (retain, nonatomic) YYIMOrgManager *orgManager;
@property (retain, nonatomic) YYIMPanManager *panManager;
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [[YYIMOrgDBHelper sharedInstance] setupDatabase];
        ...
        // org
        self.orgManager = [YYIMOrgManager sharedInstance];
        [self.orgManager activateWithJUMPStream:jumpStream delegate:multicastDelegate];
        // pan
        self.panManager = [YYIMPanManager sharedInstance];
        [self.panManager activateWithJUMPStream:jumpStream delegate:multicastDelegate];
        // attach
...
- (void)sendFileMessage:(NSString *)chatId files:(NSArray *)yyFileArray chatType:(NSString *)chatType {
    chatId = [chatId lowercaseString];
    [self.messageManager sendFileMessage:chatId files:yyFileArray chatType:chatType];
}
...
- (void)loadUserProfilesWithComplete:(void (^)(BOOL result,NSDictionary *dic, NSString *msg)) complete {
    [self.extManager loadUserProfilesWithComplete:complete];
}
...
- (void)updateUserExt:(YYUserExt *)userExt {
    [self.extManager updateUserExt:userExt];
}

- (void)updateChatGroupExt:(YYChatGroupExt *)chatGroupExt {
    [self.extManager updateChatGroupExt:chatGroupExt];
}

- (void)updatePubAccountExt:(YYPubAccountExt *)pubAccountExt {
    [self.extManager updatePubAccountExt:pubAccountExt];
}
...
- (void)setOrgEnable:(BOOL)enable {
    [self.orgManager setOrgEnable:enable];
}

- (YYOrg *)getRootOrg {
    return [self.orgManager getRootOrg];
}

- (YYOrg *)getOrgWithParentId:(NSString *)parentId {
    return [self.orgManager getOrgWithParentId:parentId];
}

- (void)loadOrgWithParentId:(NSString *)parentId {
    [self.orgManager loadOrgWithParentId:parentId];
}

#pragma mark -
#pragma mark pan protocol

- (void)setPanEnable:(BOOL)enable {
    [self.panManager setPanEnable:enable];
}

- (YYFile *)getDirWithId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    return [self.panManager getDirWithId:dirId fileSet:fileSet group:groupId];
}

- (NSArray *)getFilesWithDirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    return [self.panManager getFilesWithDirId:dirId fileSet:fileSet group:groupId];
}

- (void)loadFileWithDirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager loadFileWithDirId:dirId fileSet:fileSet group:groupId];
}

- (void)searchFileWithKeyword:(NSString *)keyword fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager searchFileWithKeyword:keyword fileSet:fileSet group:groupId];
}

- (void)uploadFileWithDirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId assets:(NSArray *)assetArray {
    [self.panManager uploadFileWithDirId:dirId fileSet:fileSet group:groupId assets:assetArray];
}
...
- (void)renameAttachmentWithId:(NSString *)attachId dirId:(NSString *)dirId newName:(NSString *)name fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager renameAttachmentWithId:attachId dirId:dirId newName:name fileSet:fileSet group:groupId];
}
...
- (void)moveAttachmentWithId:(NSString *)attachId dirId:(NSString *)dirId newDirId:(NSString *)newDirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager moveAttachmentWithId:attachId dirId:dirId newDirId:newDirId fileSet:fileSet group:groupId];
}

- (void)deleteAttachmentWithId:(NSString *)attachId dirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager deleteAttachmentWithId:attachId dirId:dirId fileSet:fileSet group:groupId];
}

- (void)downloadAttachmentWithId:(NSString *)attachId dirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager downloadAttachmentWithId:attachId dirId:dirId fileSet:fileSet group:groupId];
}
...
- (void)downloadAttachmentsWithId:(NSArray *)attachIdArray dirId:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    [self.panManager downloadAttachmentsWithId:attachIdArray dirId:dirId fileSet:fileSet group:groupId];
}
...
- (void)jumpStreamDidAuthenticate:(JUMPStream *)sender {
    ...
    [self loadOrgWithParentId:YM_ORG_ROOT_ID];
    [self loadFileWithDirId:YM_FILE_ROOT_ID fileSet:kYYIMFileSetPublic group:nil];
    //    [self load]
    [self goOnline];
    [self.messageManager loadOfflineMessage];
    [[self activeDelegate] didAuthenticate];
    [self loadUserProfilesWithComplete:nil];
}
...
- (void)setGroupAnnouncementWithGroup:(NSString *)groupId content:(NSString *)content complete:(void (^)(BOOL, YYChatGroup *, YYIMError *))complete {
    groupId = [groupId lowercaseString];
    [[self getChatGroupProvider] setGroupAnnouncementWithGroup:groupId content:content complete:complete];
}

- (void)addGroupAdministratorsWithGroup:(NSString *)groupId adminId:(NSArray *)adminId complete:(void (^)(BOOL result, YYChatGroup * groupInfo, YYIMError *error)) complete {
    groupId = [groupId lowercaseString];
    [[self getChatGroupProvider] addGroupAdministratorsWithGroup:groupId adminId:adminId complete:complete];
}

- (void)deleteGroupAdministratorsWithGroup:(NSString *)groupId adminId:(NSArray *)adminId complete:(void (^)(BOOL result, YYChatGroup * groupInfo, YYIMError *error)) complete {
    groupId = [groupId lowercaseString];
    [[self getChatGroupProvider] deleteGroupAdministratorsWithGroup:groupId adminId:adminId complete:complete];
}
...
- (void)updateSilenceMode:(BOOL)silenceMode complete:(void (^)(BOOL result, NSString *msg)) complete {
    [self.extManager updateSilenceMode:silenceMode complete:complete];
}
/**
 勿扰模式
...
- (void)enableUserNoDisturbance:(NSInteger)beginHour beginMinute:(NSInteger)beginMinute endHour:(NSInteger)endHour endMinute:(NSInteger)endMinute {
    [self.extManager enableUserNoDisturbance:beginHour beginMinute:beginMinute endHour:endHour endMinute:endMinute];
}
/**
 *  获取用户Profile
 *
...
- (YYUserProfileSetting *)getUserProfileSetting {
    return [self.extManager getUserProfileSetting];
}
/**
 是否临时不被打扰
...
- (BOOL)isTempNoDisturb:(NSString *)fromId {
    fromId = [fromId lowercaseString];
    return [self.extManager isTempNoDisturb:fromId];
}

- (void)setUserMute:(BOOL)mute {
    [self.extManager setUserMute:mute];
}

- (void)sendFileLogComplete:(void (^)(BOOL result, YYIMError *error))complete {
    [self.messageManager sendFileLogComplete:complete];
}
@end
