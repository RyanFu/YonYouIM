...
+ (NSString *)genGroupName:(YYUser *)user invites:(NSArray *)inviteArray {
    ...
    for (id invite in inviteArray) {
        ...
        } else if ([invite isKindOfClass:[YYOrgEntity class]]) {
            YYOrgEntity *orgEntity = (YYOrgEntity *)invite;
            [name appendString:@"、"];
            [name appendString:[orgEntity orgName]];
        }
    }
    return name;
}
...
+ (void)clearBackButtonText:(UIViewController *)viewController {
    [viewController.navigationItem setBackBarButtonItem:[[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStyleBordered target:nil action:nil]];
}
    ...
    switch ([message type]) {
        case YM_MESSAGE_CONTENT_PROMPT:{
            ...
            if ([promptType isEqualToString:@"accept_roster"]) {// 同意好友
                ...
                [str appendString:@"加入了群组"];
                simpleMessage = str;
            } else if ([promptType isEqualToString:@"cancelAdmin"]) {
                NSMutableString *str = [NSMutableString string];
                ...
                [str appendString:[operhandNameArray componentsJoinedByString:@"、"]];
                [str appendString:@"的管理员权限"];
                simpleMessage = str;
            } else if ([promptType isEqualToString:@"assignAdmin"]) {
                NSMutableString *str = [NSMutableString string];
