...
@interface ChatViewController ()<YYIMChatDelegate, YMMessageToolViewDelegate, YMMessageExtendDelegate, YYIMAssetDelegate, YYIMFileDelegate, YYIMLocationDelegate, YMMemberSelDelegate, YMDirDelegate, YMChatSelDelegate, UIDocumentInteractionControllerDelegate, UIAlertViewDelegate, AVAudioRecorderDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate, YYIMMicroVideoRecordViewDelegate, YMPubAccountMenuCustomViewDelegate, YMTableMenuDelegate> {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[YYIMConfig sharedInstance] setCurrentInterface:self.chatId];
}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [[YYIMConfig sharedInstance] setCurrentInterface:nil];
}
...
- (void)bubbleLongPressWithUserInfo:(NSDictionary *)userInfo {
    ...
    // 保存到云盘
    if ([message type] == YM_MESSAGE_CONTENT_FILE || [message type] == YM_MESSAGE_CONTENT_IMAGE) {
        UIMenuItem *itemSaveAttach = [[UIMenuItem alloc] initWithTitle:@"保存到云盘" action:@selector(handleSaveAttach:)];
        [array addObject:itemSaveAttach];
    }
...
- (void)handleSaveAttach:(id)sender {
    FolderTabBarController *folderViewController = [[FolderTabBarController alloc] initWithNibName:@"FolderTabBarController" bundle:nil];
...
- (void)didSelectDir:(NSString *)dirId fileSet:(YYIMFileSet)fileSet group:(NSString *)groupId {
    if (self.curMessage) {
        NSString *attachId = [[self.curMessage getMessageContent] fileAttachId];
        [[YYIMChat sharedInstance].chatManager createAttachmentWithId:attachId dirId:dirId fileSet:fileSet group:nil];
        [self showHint:@"保存成功"];
        self.curMessage = nil;
    }
}
