...
// We probably shouldn't be using YMDDLog() statements within the YMDDLog implementation.
...
@interface YMDDTTYLoggerColorProfile : NSObject {
@public
    int mask;
    int context;
...
static YMDDTTYLogger *sharedInstance;
...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor
{
    ...
    for (OSColor *color in colors)
    {
        ...
        NSLogVerbose(@"YMDDTTYLogger: %3lu : %.3f,%.3f,%.3f & %.3f,%.3f,%.3f = %.6f",
        ...
        if (distance < lowestDistance)
        {
            ...
            NSLogVerbose(@"YMDDTTYLogger: New best index = %lu", (unsigned long)bestIndex);
        }
...
+ (void)initialize
{
    ...
    if (!initialized)
    {
        ...
        NSLogInfo(@"YMDDTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
        NSLogInfo(@"YMDDTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
        NSLogInfo(@"YMDDTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
...
- (BOOL)colorsEnabled
{
    // The design of this method is taken from the YMDDAbstractLogger implementation.
    // For extensive documentation please refer to the YMDDAbstractLogger implementation.
    ...
    dispatch_queue_t globalLoggingQueue = [YMDDLog loggingQueue];
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        YMDDTTYLoggerColorProfile *newColorProfile =
            [[YMDDTTYLoggerColorProfile alloc] initWithForegroundColor:txtColor
                                                     backgroundColor:bgColor
                                                                flag:mask
                                                             context:ctxt];
        
        NSLogInfo(@"YMDDTTYLogger: newColorProfile: %@", newColorProfile);
        
        NSUInteger i = 0;
        for (YMDDTTYLoggerColorProfile *colorProfile in colorProfilesArray)
        {
    ...
    // The design of the setter logic below is taken from the YMDDAbstractLogger implementation.
    // For documentation please refer to the YMDDAbstractLogger implementation.
...
- (void)logMessage:(YMDDLogMessage *)logMessage
{
    ...
    if (logMsg)
    {
        ...
        YMDDTTYLoggerColorProfile *colorProfile = nil;
        ...
        if (colorsEnabled)
        {
            ...
            if (colorProfile == nil)
            {
                for (YMDDTTYLoggerColorProfile *cp in colorProfilesArray)
                {
...
@implementation YMDDTTYLoggerColorProfile
...
- (instancetype)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(int)aMask context:(int)ctxt
{
    if ((self = [super init]))
    {
        ...
        if (fgColor)
        {
            [YMDDTTYLogger getRed:&r green:&g blue:&b fromColor:fgColor];
        ...
        if (bgColor)
        {
            [YMDDTTYLogger getRed:&r green:&g blue:&b fromColor:bgColor];
        ...
        if (fgColor && isaColorTTY)
        {
            ...
            fgCodeIndex = [YMDDTTYLogger codeIndexForColor:fgColor];
        ...
        if (bgColor && isaColorTTY)
        {
            ...
            bgCodeIndex = [YMDDTTYLogger codeIndexForColor:bgColor];
...
- (NSString *)description
{
    return [NSString stringWithFormat:
            @"<YMDDTTYLoggerColorProfile: %p mask:%i ctxt:%i fg:%u,%u,%u bg:%u,%u,%u fgCode:%@ bgCode:%@>",
