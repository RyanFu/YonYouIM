...
@property (retain, nonatomic) YYUserProfileSetting *settings;
...
- (void)viewDidLoad {
    ...
    self.settings = [[YYIMChat sharedInstance].chatManager getUserProfileSetting];
}
    ...
    switch (indexPath.section) {
        ...
        case 1:
            ...
            switch (indexPath.row) {
                case 0:
                    [cell setName:@"新消息提醒"];
                    [cell setSwitchState:[self.settings isRemind]];
                    [cell.switchControl setTag:1];
                    cell.switchControl.enabled = NO;
                    break;
                case 1:
                    [cell setName:@"铃声"];
                    [cell setSwitchState:[self.settings isVoiceInApp]];
                    [cell.switchControl setTag:2];
                    cell.switchControl.enabled = YES;
                    break;
                case 2:
                    [cell setName:@"振动"];
                    [cell setSwitchState:[self.settings isVibrateInApp]];
                    [cell.switchControl setTag:3];
                    cell.switchControl.enabled = YES;
                    break;
                case 3:
                    [cell setName:@"显示新消息详情"];
                    [cell setSwitchState:[self.settings isPreview]];
...
- (void)didSwitch:(UISwitch *)switchControl {
    switch ([switchControl tag]) {
        case 1:
            break;
        case 2:
            [self.settings setIsVoiceInApp:[switchControl isOn]];
            [[YYIMChat sharedInstance].chatManager setUserMute:[switchControl isOn]];
            break;
        case 3:
            [self.settings setIsVibrateInApp:[switchControl isOn]];

            break;
        case 4:
            [self.settings setIsPreview:[switchControl isOn]];
