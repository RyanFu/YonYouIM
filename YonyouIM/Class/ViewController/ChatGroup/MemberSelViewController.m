...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0,CGRectGetWidth([UIScreen mainScreen].bounds) , 48)];
    button.backgroundColor = [UIColor whiteColor];
    [button addTarget:self action:@selector(selectBtn:) forControlEvents:UIControlEventTouchUpInside];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(15, (button.frame.size.height - 20) / 2, button.frame.size.width - 30, 20)];
    label.text = @"所有人";
...
- (void)selectBtn:(UIButton *)sender {
    YYChatGroupMember *member = [YYChatGroupMember allUser];
    member.groupId = self.groupId;
    
    if ([self isAllowMultipleSelect]) {
        if ([self.delegate respondsToSelector:@selector(memberSelController:identifiy:didSelMembers:)]) {
            [self.delegate memberSelController:self identifiy:self.identifiy didSelMembers:@[member]];
        }
    } else {
