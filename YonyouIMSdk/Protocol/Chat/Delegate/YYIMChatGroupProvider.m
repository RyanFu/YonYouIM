        ...
        if (result && [token tokenStr]) {
            ...
            [manager GET:urlString parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
                ...
                for (NSDictionary *item in items) {
                    ...
                    [member setAssignAdminTime:[[item objectForKey:@"assignAdminTime"] integerValue]];
...
- (void)handleLoadChatGroupResponse:(JUMPPacket *)jumpPacket withInfo:(id <JUMPTrackingInfo>)info {
    ...
    for (NSDictionary *item in items) {
        ...
        NSDictionary *announcementDic = item[@"announcement"];
        if (announcementDic && announcementDic.count > 0) {
            [group setAnnouncementCreator:announcementDic[@"creator"]];
            [group setAnnouncementContent:announcementDic[@"content"]];
            [group setAnnouncementTs:[announcementDic[@"ts"] integerValue]];
        }
        // members
    ...
    for (NSDictionary *item in items) {
        ...
        for (NSDictionary *mitem in members) {
            ...
            [member setAssignAdminTime:[[mitem objectForKey:@"assignAdminTime"] integerValue]];
            [memberArray addObject:member];
        }
        // 更新数据库
...
- (BOOL)didReceiveGroupUpdatePush:(JUMPIQ *)iq {
    ...
    NSDictionary *announcementDic = [iq objectForKey:@"announcement"];
    if (announcementDic && announcementDic.count > 0) {
        [group setAnnouncementContent:announcementDic[@"content"]];
        [group setAnnouncementCreator:announcementDic[@"creator"]];
        [group setAnnouncementTs:[announcementDic[@"ts"] integerValue]];
    ...
    if ([type isEqualToString:@"create"]) {
        ...
        [dic setObject:group.announcementContent ?: @"" forKey:@"content"];
        [dic setObject:@"atuser" forKey:@[@"im_atall"]];
        operhand = dic;
        [self injectGroupOperateMessage:groupId type:type operator:operator operhand:operhand dateline:dateline otherInfo:nil];
    }
...
- (BOOL)didReceiveFaceGroupUpdatePush:(JUMPIQ *)iq {
    ...
    if ([operationType isEqualToString:@"join"] || [operationType isEqualToString:@"participation"]) {
...
- (void)setGroupAnnouncementWithGroup:(NSString *)groupId content:(NSString *)content complete:(void (^)(BOOL, YYChatGroup *, YYIMError *))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [YYIMJUMPHelper genAvailableTokenWithComplete:^(BOOL result, YYToken *token, YYIMError *tokenError) {
            if (result && [token tokenStr]) {
                NSString *urlString = [[YYIMConfig sharedInstance] getGroupAnnouncement:groupId];
                ...
                [manager PUT:urlString parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                    YYIMLogDebug(@"更改群公告成功");
                    YYChatGroup *message = [[YYIMDBHelper sharedInstance] getChatGroupWithId:groupId];
                    complete(YES, message, nil);
                } failure:^(NSURLSessionDataTask * _Nullable task, id  _Nonnull responseObject, NSError * _Nonnull error) {
                    YYIMLogDebug(@"更改群公告失败");
                    complete(NO, nil, [YYIMError errorWithNSError:error]);
                }];
            } else {
                YYIMLogDebug(@"token获取失败");
                complete(NO, nil, tokenError);
            }
        }];
    });
...
- (void)addGroupAdministratorsWithGroup:(NSString *)groupId adminId:(NSArray *)adminId complete:(void (^)(BOOL result, YYChatGroup * groupInfo, YYIMError *error)) complete {
    [self changGroupAdministratorsWithGroup:groupId adminId:adminId isAdd:YES complete:complete];
}
...
- (void)deleteGroupAdministratorsWithGroup:(NSString *)groupId adminId:(NSArray *)adminId complete:(void (^)(BOOL result, YYChatGroup * groupInfo, YYIMError *error)) complete {
    [self changGroupAdministratorsWithGroup:groupId adminId:adminId isAdd:NO complete:complete];
}
- (void) changGroupAdministratorsWithGroup:(NSString *)groupId adminId:(NSArray *)adminId isAdd:(BOOL)isAdd complete:(void (^)(BOOL result, YYChatGroup * groupInfo, YYIMError *error)) complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [YYIMJUMPHelper genAvailableTokenWithComplete:^(BOOL result, YYToken *token, YYIMError *tokenError) {
            if (result && [token tokenStr]) {
                NSString *urlString = @"";
                if (isAdd) {
                    urlString = [[YYIMConfig sharedInstance] getAddGroupAdministrators:groupId];
                } else {
                    urlString = [[YYIMConfig sharedInstance] getDeleteGroupAdministrators:groupId];
                }
                if ([YYIMStringUtility isEmpty:urlString]) {
                    return;
                }
                ...
                NSMutableDictionary *params = [NSMutableDictionary dictionary];
                [params setObject:adminId forKey:@"adminIds"];
                ...
                urlString = [NSString stringWithFormat:@"%@?token=%@", urlString, [token tokenStr]];
                [manager PUT:urlString parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                    YYIMLogDebug(@"添加管理员成功");
                    YYChatGroup *message = [[YYIMDBHelper sharedInstance] getChatGroupWithId:groupId];
                    complete(YES, message, nil);
                } failure:^(NSURLSessionDataTask * _Nullable task, id  _Nonnull responseObject, NSError * _Nonnull error) {
                    YYIMLogDebug(@"添加管理员失败");
                    complete(NO, nil, [YYIMError errorWithNSError:error]);
                }];
            } else {
                YYIMLogDebug(@"token获取失败");
...
- (void)getReadMessageNumberWithGroupId:(NSString *)groupId {
    NSString *urlString = [[YYIMConfig sharedInstance] getReadMessageNumberWithGroupId:groupId];
    if ([YYIMStringUtility isEmpty:urlString]) {
        return;
    }
...
- (void)getUserReadMessageNumberWithGroupId:(NSString *)groupId packetId:(NSString *)packetId {
    NSString *urlString = [[YYIMConfig sharedInstance] getUserReadMessageNumberWithGroupId:groupId packetId:packetId];
    if ([YYIMStringUtility isEmpty:urlString]) {
        return;
    }
