...
@interface YYIMMessageManager ()<JUMPStreamDelegate> {
    ...
    NSMutableDictionary *logDictonary;// 记录日志返回的ID
}
...
- (void)sendMicroVideoMessage:(NSString *)chatId filePath:(NSString *)filePath thumbPath:(NSString *)thumbPath chatType:(NSString *)chatType {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *resFullPath   = [YYIMResourceUtility fullPathWithResourceRelaPath:filePath];
        NSURL *fileURL = [NSURL fileURLWithPath:resFullPath];
        AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:fileURL options:nil];
        if (asset) {
            [content setDuration:CMTimeGetSeconds(asset.duration)];
        }
...
- (void)sendFileMessage:(NSString *)chatId files:(NSArray *)yyFileArray chatType:(NSString *)chatType {
    NSParameterAssert(chatId != nil);
    NSParameterAssert(yyFileArray.count > 0);
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (YYFile *file in yyFileArray) {
            if (![file isKindOfClass:[YYFile class]]) {
                continue;
            }
            if ([file isDir]) {
                continue;
            ...
            [content setFileAttachId:[file fileId]];
            [content setFileName:[file fileName]];
            [content setFileExtension:[[file fileName] pathExtension]];
            [content setFileSize:[file fileSize]];
            [self sendMessage:chatId contentType:YM_MESSAGE_CONTENT_FILE content:content chatType:chatType];
        }
    });
}
...
- (void)handleMucOnlineDeliverPacket:(JUMPMessage *)messagePacket {
    ...
    } else if ([category isEqualToString:@"cancelAdmin"] || [category isEqualToString:@"assignAdmin"]) {
        if (attributes && attributes.count > 0) {
            NSArray *userIds = [attributes objectForKey:@"userJids"];
            JUMPJID *groupJid = [messagePacket from];
            NSString *groupId = [YYIMJUMPHelper parseUser:[groupJid bare]];
            
            NSTimeInterval time = 0;
            if (![attributes.allKeys containsObject:@"assignAdminTime"] || !attributes[@"assignAdminTime"] || [attributes[@"assignAdminTime"] integerValue] == 0) {
                time=[[NSDate date] timeIntervalSince1970] * 1000;
            } else {
                time = [attributes[@"assignAdminTime"] integerValue];
            }
            
            NSMutableArray *array = [NSMutableArray arrayWithArray:[[YYIMChatGroupMemberDBHelper sharedInstance] getChatGroupMembersWithGroupId:groupId]];
            NSMutableArray *operhandArray = [NSMutableArray array];
            for (NSString *str in userIds) {
                NSString *userId = [[str componentsSeparatedByString:@"."] firstObject];
                for (YYChatGroupMember *member in array) {
                    if ([userId isEqual:member.memberId]) {
                        if ([category isEqualToString:@"assignAdmin"]) {
                            [member setMemberRole:@"admin"];
                        } else {
                            [member setMemberRole:@"member"];
                        }
                        [member setAssignAdminTime:time];
            ...
            [[YYIMChatGroupMemberDBHelper sharedInstance] batchUpdateChatGroupMember:groupId members:array];
            [[self activeDelegate] didChatGroupMemberUpdate:groupId];
            
            [self injectGroupOperateMessage:groupId type:category operator:[YYIMJUMPHelper parseUser:[groupJid resource]] operhand:operhandArray dateline:[NSNumber numberWithDouble:time] otherInfo:nil];
        ...
        [manager GET:urlString parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            for (NSDictionary *packetDic in packetArray) {
                ...
                if (error) {
                    ...
                    NSInteger packetVersion = [contentDic[@"packetVersion"] integerValue];
                    [[YYIMDBHelper sharedInstance] updateMessageDeliveredWithPacketVersion:packetVersion];
                }
            }
            // 离线消息总量
...
- (void)getChatRecentContact {
    NSString *urlString = [[YYIMConfig sharedInstance] getUserRecentContact];
    if ([YYIMStringUtility isEmpty:urlString]) {
        return;
    }
...
- (void)setPubAccountsWithAccountID:(NSString *)accountId msgModel:(NSInteger)msgModel {
    NSString *urlString = [[YYIMConfig sharedInstance] getPubAccountMessageAccount:accountId msgModel:msgModel];
    if ([YYIMStringUtility isEmpty:urlString]) {
        return;
    }
...
// logs 路径
- (NSString *)getLogPath {
    NSArray *cashArray = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *baseDir = ([cashArray count] > 0) ? [cashArray objectAtIndex:0] : nil;
    NSString *logsDirectory = [baseDir stringByAppendingPathComponent:@"Logs"];

    return logsDirectory;
}

- (void)sendFileLogComplete:(void (^)(BOOL result, YYIMError *error))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (logDictonary) {
            [logDictonary removeAllObjects];
        } else {
            logDictonary = [NSMutableDictionary dictionaryWithCapacity:4];
        }
        
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *fileName = [NSString stringWithFormat:@"%@.sqlite",[[YYIMConfig sharedInstance] getFullUser]];
        NSArray *array = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
        NSString *path = [array lastObject];
        path = [path stringByAppendingPathComponent:@"ymchat.sqlite"];
        
        long long fileSize;
        NSDictionary *fileAttributes = [fileManager attributesOfItemAtPath:path error:nil];
        fileSize = [fileAttributes fileSize];

        NSString *logsDirectory = [self getLogPath];
        NSArray *logArray = [fileManager contentsOfDirectoryAtPath:logsDirectory error:nil];
        ...
        for (NSString *dateString in logArray) {
            NSString * string = [dateString stringByReplacingOccurrencesOfString:[self applicationName] withString:@""];
            NSString *dataStr = [string stringByReplacingOccurrencesOfString:@".log" withString:@""];
            dataStr = [dataStr stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        ...
        // 最多上传10个文件
        NSInteger t = (tempArray.count > 10 ? 10 : tempArray.count);
        // 数据库上传
        [self sendFileLogPath:path fileName:fileName size:fileSize Complete:^(BOOL result,NSString *string, YYIMError *error) {
            [self fileLogithType:YES number:t + 1 attachID:string fileName:fileName complete:complete];
        }];
        //  log 日志上传
        for (int i = 0; i < t; i++) {
            NSDate *data = tempArray[i];
            NSString *stringPath = tempDictionary[data];
            stringPath = [logsDirectory stringByAppendingPathComponent:stringPath];

            NSString *createDir = [logsDirectory stringByAppendingPathComponent:@"test"];
            BOOL isDir = NO;
            ...
            createDir = [createDir stringByAppendingPathComponent:tempDictionary[data]];
            [fileManager copyItemAtPath:stringPath toPath:createDir error:nil];
            stringPath = createDir;

            NSString *stringName = [NSString stringWithFormat:@"%@.%@.log",[[YYIMConfig sharedInstance] getFullUser],[formatter stringFromDate:data]];
            
            fileSize = 0;
            fileAttributes = [fileManager attributesOfItemAtPath:stringPath error:nil];
            fileSize = [fileAttributes fileSize];
            [self sendFileLogPath:stringPath fileName:stringName size:fileSize Complete:^(BOOL result,NSString *string, YYIMError *error) {
                [self fileLogithType:NO number:t + 1 attachID:string fileName:stringName complete:complete];
            }];
        }
    });
}
// 上传日志文件
- (void)sendFileLogPath:(NSString *)filePath fileName:(NSString *)fileName size:(long long)fileSize Complete:(void (^)(BOOL result,NSString *string, YYIMError *error))complete {
    [YYIMHttpUtility uploadResourceWithSourcePath:filePath fileName:fileName fileSize:fileSize receiver:@"" mediaType:kYYIMUploadMediaTypeDoc isOriginal:YES completion:^(NSInteger resultCode, YYIMError *error, id responseObject) {
        if (resultCode == 200) {
            NSString *attachId = @"";
            if ([responseObject isKindOfClass:[NSData class]]) {
                NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
                 attachId = [dic objectForKey:@"attachId"];
            } else if ([responseObject isKindOfClass:[NSDictionary class]]) {
                attachId = [(NSDictionary *)responseObject objectForKey:@"attachId"];
            }
            complete(YES, attachId, nil);
        } else {
            YYIMLogError(@"filePath:%@\n error:%@",filePath,error.errorMsg);
            complete(NO, nil, error);
        }
    }];
}
// 记录上传日志id 并且将id 上传
- (void)fileLogithType:(BOOL)isSql number:(NSInteger)number attachID:(NSString *)attachId fileName:(NSString *)fileName complete:(void (^)(BOOL result, YYIMError *error))complete {
    NSInteger curentNum = [logDictonary[@"curentNum"] integerValue];
    curentNum ++;
    [logDictonary setObject:[NSString stringWithFormat:@"%ld",(long)curentNum] forKey:@"curentNum"];
    if (attachId && attachId.length > 0) {
        if (isSql) {
            NSMutableArray *array = [NSMutableArray arrayWithArray:logDictonary[@"dbFiles"]];
            [array addObject:@{@"attachId" : attachId, @"fileName" : fileName}];
            [logDictonary setObject:array forKey:@"dbFiles"];
        } else {
            NSMutableArray *array = [NSMutableArray arrayWithArray:logDictonary[@"logFiles"]];
            [array addObject:@{@"attachId" : attachId, @"fileName" : fileName}];
            [logDictonary setObject:array forKey:@"logFiles"];
        }
    }
    if (curentNum == number) {
        // 删除copy的上传文件的文件夹
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *createDir = [[self getLogPath] stringByAppendingPathComponent:@"test"];
        BOOL isDir = NO;

        BOOL existed = [fileManager fileExistsAtPath:createDir isDirectory:&isDir];
        if (isDir && existed) {
            [fileManager removeItemAtPath:createDir error:nil];
        }

        [YYIMJUMPHelper genAvailableTokenWithComplete:^(BOOL result, YYToken *token, YYIMError *tokenError) {
            if (result && [token tokenStr]) {
                NSMutableString *urlString = [NSMutableString stringWithString:[[YYIMConfig sharedInstance] getUserFeedback]];
                [urlString appendString:[NSString stringWithFormat:@"?token=%@", [token tokenStr]]];
                if ([YYIMStringUtility isEmpty:urlString]) {
                    YYIMLogError(@"getUserFeedback nil");
                    return;
                }
                
                NSMutableDictionary *params = [NSMutableDictionary dictionary];
                if (logDictonary && [logDictonary.allKeys containsObject:@"curentNum"]) {
                    [logDictonary removeObjectForKey:@"curentNum"];
                }
                [params setObject:logDictonary forKey:@"contents"];
                ...
                [manager POST:urlString parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                    ...
                    YYIMLogError(@"update log success: %@-----%@",params,dic);
                } failure:^(NSURLSessionDataTask * _Nullable task, id  _Nonnull responseObject, NSError * _Nonnull error) {
                    if (complete) {
                        complete(NO, [YYIMError errorWithNSError:error]);
                    }
                    YYIMLogError(@"update log error：%@", error.localizedDescription);
                }];
            } else {
                if (complete) {
                    complete(NO, tokenError);
                }
                YYIMLogError(@"token error：%@", tokenError.errorMsg);
            }
        }];
    }
}
