...
- (YYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation YYFMDatabase
...
#pragma mark YYFMDatabase instantiation and deallocation
...
- (void)dealloc {
    [self close];
    YYFMDBRelease(_openResultSets);
    YYFMDBRelease(_cachedStatements);
    YYFMDBRelease(_dateFormat);
    YYFMDBRelease(_databasePath);
    YYFMDBRelease(_openFunctions);
...
+ (NSString*)YYFMDBUserVersion {
    return @"2.3";
}

// returns 0x0230 for version 2.3.  This makes it super easy to do things like:
// /* need to make sure to do X with YYFMDB version 2.3 or later */
// if ([FMDatabase YYFMDBVersion] >= 0x0230) { … }

+ (SInt32)YYFMDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 YYFMDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self YYFMDBUserVersion];
        ...
        char *e = nil;
        YYFMDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    

    return YYFMDBVersionVal;
}
...
static int YYFMDBDatabaseBusyHandler(void *f, int count) {
    YYFMDatabase *self = (__bridge YYFMDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &YYFMDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to YYFMDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = YYFMDBReturnAutoreleased([_openResultSets copy]);
...
- (YYFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(YYFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(YYFMStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = YYFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = YYFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    YYFMDBAutorelease(_dateFormat);
    _dateFormat = YYFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    YYFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The YYFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The YYFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The YYFMDatabase %@ is not open.", self);
        
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The YYFMDatabase %@ is not open.", self);
            abort();
        }
    #endif
...
- (YYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    YYFMStatement *statement  = 0x00;
    YYFMResultSet *rs         = 0x00;
    ...
    YYFMDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[YYFMStatement alloc] init];
    ...
    rs = [YYFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (YYFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    YYFMStatement *cachedStmt  = 0x00;
    
    if (_traceExecution && sql) {
        YYIMLogError(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                YYIMLogError(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                YYIMLogError(@"DB Query: %@", sql);
                YYIMLogError(@"DB Path: %@", _databasePath);
            }
    ...
    if (idx != queryCount) {
        YYIMLogError(@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            YYIMLogError(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            YYIMLogError(@"DB Query: %@", sql);
        }
    }
        ...
        // uh oh.
        if (_logsErrors) {
            YYIMLogError(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            YYIMLogError(@"DB Query: %@", sql);
        }
    }
    else {
        // wtf?
        if (_logsErrors) {
            YYIMLogError(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            YYIMLogError(@"DB Query: %@", sql);
        }
    }
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[YYFMStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        YYFMDBRelease(cachedStmt);
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            YYIMLogError(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            YYIMLogError(@"DB Query: %@", sql);
        }
    }
...
int YYFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int YYFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(YYFMDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? YYFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *YYFMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", YYFMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", YYFMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", YYFMDBEscapeSavePointName(name)];
    BOOL worked = [self executeUpdate:sql];
...
void YYFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void YYFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = YYFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &YYFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &YYFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
