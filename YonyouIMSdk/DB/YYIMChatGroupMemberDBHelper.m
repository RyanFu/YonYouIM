...
- (void) updateDatabase {
    ...
    switch (dbVersion) {
        ...
        case YYIM_DB_VERSION_16:
            [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
                [db executeUpdate:YYIM_CHATGROUPMEMBER_ADD_TS];
    ...
    [[self getDBQueue] inTransaction:^(YYFMDatabase *db, BOOL *rollback) {
        ...
        for (YYChatGroupMember *member in memberArray) {
            ...
            if ([memberIdArray containsObject:[member memberId]]) {
                ...
                [array addObject:[NSNumber numberWithLongLong:[member assignAdminTime]]];
                ...
                [db executeUpdate:YYIM_CHATGROUPMEMBER_UPDATE_NEW withArgumentsInArray:array];
                ...
                [array addObject:[YYIMStringUtility notNilString:[member memberRole]]];
                [array addObject:[NSNumber numberWithLongLong:[member assignAdminTime]]];
                [db executeUpdate:YYIM_CHATGROUPMEMBER_INSERT_NEW withArgumentsInArray:array];
            }
        }
    ...
    [[self getDBQueue] inDatabase:^(YYFMDatabase *db) {
        ...
        @try {
            while ([rs next]) {
                ...
                [member setAssignAdminTime:[[rs stringForColumn:@"assign_admints"] integerValue]];
